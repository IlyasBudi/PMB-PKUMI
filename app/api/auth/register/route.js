...
export async function POST(request) {
  ...
  try {
    ...
    // FIND USER TYPE
    // Temukan Enumeration untuk user mahasiswa
    const userType = await Enumeration.findOne({
      where: {
        code: constants.ENUMERATION_USERS_TYPE_MAHASISWA,
        isActive: true,
      },
    ...
    if (!userType) {
      throw {
        ...
        message: "User type mahasiswa tidak ditemukan di tabel Enumeration",
      };
    }
    ...
    let transporter = nodemailer.createTransport({
      ...
      secure: false, // pakai STARTTLS di port 587
      auth: {
