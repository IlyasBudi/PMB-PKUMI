...
export async function POST(request) {
  ...
  try {
      ...
      include: [
        {
          ...
          where: {
            code: {
              [Op.or]: [
                constants.ENUMERATION_USERS_TYPE_ADMIN,
                constants.ENUMERATION_USERS_TYPE_MAHASISWA,
              ],
            },
          },
          include: [
    ...
    // })
    cookies().set(process.env.NEXT_PUBLIC_APP_TOKEN_NAME, token, {
      secure: false, // ⚠️ gunakan false karena masih pakai http
      httpOnly: false, // agar bisa dibaca dari client-side
      path: "/", // supaya berlaku di semua route
    });
